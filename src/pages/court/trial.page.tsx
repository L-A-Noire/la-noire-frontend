// // src/pages/court/trial.page.tsx
...
//   // Get caseId from location state (passed from CourtDashboardPage)
...
//   // Get suspect details from the suspect_crime response
...
//   // Reset form when verdict changes
//   useEffect(() => {
//     if (verdict === "pending") {
//       setPunishmentType("");
//       setTitle("");
...
//   // Mutation: Issue Punishment (for guilty verdict)
...
//         // Don't include case - let backend derive it
...
//     mutationFn: async () => {
//       // Update suspect status to innocent
//       await updateSuspectCrimeStatus(suspectCrimeId, "innocent");

//       // Case remains open (no punishment)
//       return { success: true };
//     },
//     onSuccess: () => {
//       queryClient.invalidateQueries({ queryKey: ["suspect-crimes"] });
...
export const TrialPage = () => {
  ...
  const punishmentMutation = useMutation({
    mutationFn: async () => {
      ...
      // Remove 'case' from the payload since the Punishment model doesn't have it
      const payload = {
        suspect_crime: suspectCrimeId,
        // case: caseId,  // REMOVE THIS LINE
        punishment_type: punishmentType,
        title,
        description,
        amount: amount || null,
