...
export function RewardMenu() {
    ...
    const canAccessRewards = session && POLICE_ROLES.includes(session.user.role_title);
    ...
    const verifyMutation = useMutation({
        ...
        onSuccess: (data) => {
            if (data.valid && data.reward) {
                ...
                setVerificationResult({
                    isValid: false,
                    error: data.message || "Invalid reward code or national ID combination",
                });
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Verify a reward by entering both the unique reward code and the recipient's national ID.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-6 py-4">
                    ...
                    {!verificationResult && (
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <Label htmlFor="reward-code" className="flex items-center gap-2">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    The unique code provided to the informant when the reward was issued
                                </p>
                            </div>
                            ...
                            {verificationResult.isValid && verificationResult.reward ? (
                                <>
                                    <div className="bg-green-50 dark:bg-green-950/20 p-4 rounded-lg border border-green-200 dark:border-green-900 flex items-start gap-3">
                                        <HugeiconsIcon icon={CheckmarkCircle01Icon} className="h-5 w-5 text-green-600 mt-0.5" />
                                        <div>
                                            <p className="font-semibold text-green-800 dark:text-green-300">Reward Verified</p>
                                        ...
                                        <CardContent className="p-4 space-y-4">
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    ...
                                                    <p className="font-semibold text-lg">
                                                        {verificationResult.reward.recipient_details.first_name}{" "}
                                                        {verificationResult.reward.recipient_details.last_name}
                                                    </p>
                                                    <p className="text-sm text-muted-foreground">
                                                        National ID: {verificationResult.reward.recipient_details.national_id}
                                                    </p>
                                                </div>
                                                <Badge
                                                    variant={verificationResult.reward.is_claimed ? "outline" : "default"}
                                                    className={
                                                ...
                                                >
                                                    {verificationResult.reward.is_claimed ? "Claimed" : "Available"}
                                                </Badge>
                                            </div>
                                            ...
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <p className="text-sm">
                                                        {format(new Date(verificationResult.reward.created_at), "MMM dd, yyyy")}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 text-xs">
                                                <div>
                                                    ...
                                                    <p className="font-mono truncate">{verificationResult.reward.unique_code}</p>
                                                </div>
                                                <div>
                                                    <p className="text-muted-foreground">Issued By</p>
                                                    <p>{verificationResult.reward.created_by_details?.username || "Unknown"}</p>
                                                </div>
                                            </div>
                                            ...
                                            {!verificationResult.reward.is_claimed ? (
                                                <div className="space-y-3 mt-2 pt-2 border-t">
                                                    <div className="space-y-2">
                                                        <Label htmlFor="full-name">Recipient Full Name (for payment)</Label>
                                                        <Input
                                                            id="full-name"
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        >
                                                            ...
                                                            {claimMutation.isPending ? "Processing..." : "Process Payment"}
                                                        </Button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="bg-amber-50 dark:bg-amber-950/20 p-3 rounded border border-amber-200 dark:border-amber-900">
                                                    <p className="text-sm text-amber-800 dark:text-amber-200 flex items-center gap-2">
                                                        ...
                                                        {verificationResult.reward.claimed_at
                                                            ? format(new Date(verificationResult.reward.claimed_at), "PPP")
                                                            : "an unknown date"}
                                                    </p>
                                                </div>
                                            )}
                                ...
                                <>
                                    <div className="bg-red-50 dark:bg-red-950/20 p-4 rounded-lg border border-red-200 dark:border-red-900 flex items-start gap-3">
                                        <HugeiconsIcon icon={AlertCircleIcon} className="h-5 w-5 text-red-600 mt-0.5" />
                                        <div>
                                            <p className="font-semibold text-red-800 dark:text-red-300">Verification Failed</p>
                                            <p className="text-sm text-red-700 dark:text-red-400">
                                                {verificationResult.error || "The reward code does not match this national ID"}
                                            </p>
                                        </div>
                                    </div>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-950/20 p-4 rounded-lg border border-blue-200 dark:border-blue-900">
                        ...
                        <ul className="text-xs text-blue-800 dark:text-blue-200 space-y-1">
                            <li>• Informants receive a unique reward code when their tip is approved</li>
                            <li>• Verify the reward by entering both the code and their national ID</li>
                            <li>• The system will confirm if they match and display the reward amount</li>
